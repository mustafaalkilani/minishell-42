...
check "brace form expands like plain form" 'echo ${USER}'
check "brace form glued to a literal suffix" 'echo ${USER}abc'
check "brace form inside double quotes" 'echo "${USER}"'
check "brace form inside single quotes stays literal" "echo '\${USER}'"
check "brace form for unset variable" 'echo [${NOSUCHXYZ}]'
check "brace form of \$?" 'true' 'echo ${?}'
check "brace form splits when unquoted" 'export S="a b c"' 'echo [${S}]'
check "brace form does not split when quoted" 'export S="a b c"' 'echo "[${S}]"'
check "brace form in a redirection target" 'export F=heredoc_target_x' 'echo hi > ${F}' 'cat ${F}'
...
check "cd with an empty operand is a silent no-op" 'cd ""' 'echo $?' 'pwd'
check "empty cd does not disturb OLDPWD" 'cd /tmp' 'cd ""' 'cd -'
